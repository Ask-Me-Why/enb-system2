...
<template>
    ...
    <div class="mi-tab-btns" @click="deleteInfo">
...
export default {
  ...
  props:['id'],
	...
	data() {
		return {
			...
			params: {
				mode: 2,
				token:'KsP3Rs_i6inzabyLWEGJqNZH-UOLb_FnE8R3C4wA7nJ2WT2TBUAuvo4S53Yc'
			},
      text:'',
  ...
  methods:{
    ...
    deleteInfo(){
      askDialogConfirm({
        content: `删除该赛事资料将无法恢复，是否删除?`,
        title: '提示',
        theme:'enb-confirm'
      },(vm)=>{
        this.$store.dispatch('ajaxRequestStart');
        this.matchApi.remove(this.id).then(r=>{
          this.$store.dispatch('ajaxRequestEnd');
          if(r.data.code != 200) return;
          vm.close();
          askDialogToast({msg:`已成功删除当前资料！`,time:2000,class:'success'});
          setTimeout("location.reload();",500)
        },err=>{
          this.$store.dispatch('ajaxRequestEnd');
        })
